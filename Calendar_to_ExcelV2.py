...
worksheetOP= workbook.add_worksheet("Operation Plan")
...
worksheetOP.write("A1", "Camp - Operational Plan", bold)
worksheetOP.write("A2", "STEM Enrichment Programmes Data")
worksheetOP.write("A4", "Arranged", bold)
worksheetOP.write("B4", "Paperwork Sent", bold)
worksheetOP.write("C4", "Quarter", bold)
worksheetOP.write("D4", "Hub", bold)
worksheetOP.write("E4", "Programme", bold)
worksheetOP.write("F4", "School Name", bold)
worksheetOP.write("G4", "School Number", bold)
worksheetOP.write("H4", "Start Date", bold)
worksheetOP.write("I4", "Participants", bold)
worksheetOP.write("J4", "Workshops Delivered?", bold)
worksheetOP.write("K4", "Workshops", bold)
worksheetOP.write("L4", "End Date", bold)
...
worksheetOP.set_column(0, 4, 10)
worksheetOP.set_column(5, 5, 20)
worksheetOP.set_column(6, 7, 12)
worksheetOP.set_column(8, 13, 12)
worksheetOP.set_column(10, 10, 40)
...
# Manages the quarter and dates fields in Operation Plan.
def quarter(listt,dateB,dateF):
    ...
    dayE = dateF[8:10]
    listt.insert(
        7, datetime.datetime.strptime(day + "-" + month + "-" + year, "%d-%m-%Y")
    )
    listt.insert(
        11, datetime.datetime.strptime(dayE + "-" + monthE + "-" + yearE, "%d-%m-%Y")
    )

    q=None
    if datetime.date(2018,7,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2018,11,1):
        q="Q1"
    elif datetime.date(2018,11,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,2,1):
        q="Q2"
    elif datetime.date(2019,2,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,5,1):
        q="Q3"
    elif datetime.date(2019,5,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,8,1):
        q="Q4"
    elif datetime.date(2019,8,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,11,1):
        q="Q5"
    elif datetime.date(2019,11,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,2,1):
        q="Q6"
    elif datetime.date(2019,2,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,5,1):
        q="Q7"
    elif datetime.date(2019,5,1)<=datetime.date(int(year),int(month),int(day))<datetime.date(2019,8,1):
        ...
        try:
            ...
            if event.get("summary") != None:
                if "Event" == title[1].strip():
                    ...
                    tempP = title[2].strip()
                    if tempP not in listOP:
                        listO = []
                        listOP.update({tempP:listO})
                        new=True
                    else:
                        listO=listOP.get(tempP)
                        listO[9]+=1
                        new = False
                    paperworkSent=None
                    schoolNumber=None
                    ...
                    if event["start"].get("dateTime") != None:
                        ...
                        beginDate = event["start"]["dateTime"]
                    else:
                        dateB(
                            event["start"]["date"], event["end"]["date"], listA, False
                        )
                        typee=False
                        finishDate = event["end"]["date"]
                            ...
                            if desc == "":
                                ...
                                if ":" in desc:
                                    ...
                                    if left in EVENT_INFORMATION:
                                        index = EVENT_INFORMATION.index(left)
                                        right = desc[desc.index(":") + 2 :]
                                        if left=="School Number:":
                                            schoolNumber=right
                                        if left == "Number of Pupils:":
                                            participants = right
                                        if left == "Initial Paperwork:":
                                            paperworkSent = right
                                        if index != 11:
                                            listA.pop(
                                                8 + index
                    ...
                    if(new==True):
                        blanking(listO,0)
                        listO.insert(0,'Y')
                        if paperworkSent!=None:
                            listO.insert(1, paperworkSent)
                        listO.insert(3,title[0].strip())
                        listO.insert(4, title[3].strip())
                        listO.insert(5, tempP)
                        if schoolNumber!=None:
                            listO.insert(6, schoolNumber)           

                        quarter(listO, beginDate,finishDate)
                        if participants!=None:
                            listO.pop(8)
                            listO.insert(8, participants)
                        listO.insert(9, 1)
                        if titleAll!="":
                            listO.pop(10)
                            listO.insert(10, titleAll)
                    else:
                        yearE = finishDate[0:4]
                        monthE = finishDate[5:7]
                        dayE = finishDate[8:10]
                        listO.pop(11)
                        listO.insert(11, datetime.datetime.strptime(dayE + "-" + monthE + "-" + yearE, "%d-%m-%Y"))

                        listO.pop(13)
                        if titleAll!="":
                            listO.insert(10, listO[10] + ","+titleAll)
                            listO.pop(11)
...
for listLL in listOfLists:
    writeE(listLL[0], listLL[1])


#Write Operation plan sheet.
row = 4
ppppp=list(listOP.keys())
for school in ppppp:
    #print(list(listOP.keys()))
    #print(school)
    column = 0
    abc=listOP.get(school)
    for lemeeOUT in abc:
        if isinstance(lemeeOUT, datetime.datetime):
            worksheetOP.write_datetime(row, column, lemeeOUT, cell_format)
        else:
            #print(lemeeOUT)
            worksheetOP.write(row, column, lemeeOUT, alignment)
        column += 1
    row += 1
